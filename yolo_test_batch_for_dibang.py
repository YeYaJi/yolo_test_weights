...
def detect(net, meta, image, thresh=.5, hier_thresh=.5, nms=.45):  # 置信度在这改
...
########################################################################################################################
def creat_txt(cfg_path, weight_path, voc_data, img_path, save_path):
    w = 1280
    h = 720
    net = load_net(cfg_path.encode("utf-8"), weight_path.encode("utf-8"), 0)
    meta = load_meta(voc_data.encode("utf-8"))
